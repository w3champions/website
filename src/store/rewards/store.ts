  ...
  actions: {
    ...
    async initiatePatreonOAuth(): Promise<void> {
      try {
        ...
        const patreonAuthUrl = `https://www.patreon.com/oauth2/authorize?response_type=code&client_id=${clientId}&redirect_uri=${PATREON_REDIRECT_URL}&state=${state}&scope=identity%20identity.memberships`;
