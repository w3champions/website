    ...
    <v-dialog v-model="show" max-width="500">
        <v-card>
            <v-card-title class="justify-center">
                ...
                <v-container>
                    <v-row class="pa-2" v-for="gateway in gateways" :key="gateway.id" justify="center" align="center">
                        <v-btn @click="signIn(gateway)" block x-large>
                            <v-col cols="2" align-self="center">
                                <v-img min-width="32" max-width="48" min-height="32" max-height="48" :src="require(`@/assets/flags/${gateway.id}.svg`)"></v-img>
                            </v-col>
                            <v-col cols="2" align-self="center">
                                {{gateway.name}}
                            </v-col>
