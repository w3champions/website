...
export default defineComponent({
  ...
  setup(props) {
    const mmrValues = computed(() => props.mmrRpTimeline.mmrRpAtDates.map((m) => m.mmr));
    const rpValues = computed(() => props.mmrRpTimeline.mmrRpAtDates.map((m) => m.rp));

    // Workaround: prevent dates from moving into the next day due to timezone conversion.
    const dates = computed(() => props.mmrRpTimeline.mmrRpAtDates.map((m) => startOfDay(utcToZonedTime(parseJSON(m.date), "UTC"))));
