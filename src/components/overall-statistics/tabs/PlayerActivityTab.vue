...
export default defineComponent({
  ...
  setup() {
    ...
    const selectedSeasonForMapsInitial = computed(() => rankingsStore.seasons[0]?.id?.toString() ?? "");
    const isAllMode = computed(() => selectedGamesPerDayMode.value === EGameMode.UNDEFINED);
    const race1String = computed(() => raceOptions.filter((r) => r.id == selectedMatchupRace1.value)[0].name);
    const race2String = computed(() => raceOptions.filter((r) => r.id == selectedMatchupRace2.value)[0].name);
    ...
    const seasons = computed(() => ["All", ...rankingsStore.seasons.map((s) => s.id.toString())]);

    const seasonsForMatchup = computed(() => {
    ...
    const matchupMmrOptions = computed(() => {
    ...
    const selectedGameLength = computed(() => {
      return gameLength.value?.filter(
        (g) => g.gameMode == selectedLengthMode.value
      )[0] ?? { lengths: [] };
    });

    const selectedGameHours = computed(() => {
      return popularGameHours.value.filter(
        (g) => g.gameMode == selectedPopularHourMode.value
      )[0] ?? { timeslots: [] };
    });

    const loadingGamesPerDayStats = computed(() => overallStatsStore.loadingGamesPerDayStats);
    const loadingPlayersPerDayStats = computed(() => overallStatsStore.loadingPlayersPerDayStats);
