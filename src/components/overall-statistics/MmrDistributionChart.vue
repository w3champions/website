...
export default defineComponent({
  ...
  setup(props) {
    ...
    const mmrOfLoggedInPlayer = computed(() => {
    ...
    const mmrGroupOfLoggedInPlayer = computed(() => {
    ...
    const mmrDistributionTotalPlayers = computed(() => {
      return props.mmrDistribution.distributedMmrs
        .map((d) => d.count)
        .reduce((a, b) => a + b);
    });

    const isTop50percent = computed(() => {
      const top50mmr =
        props.mmrDistribution.distributedMmrs[
    ...
    const cumulativeSumData = computed(() => {
      return props.mmrDistribution.distributedMmrs
        .slice()
        .reverse()
        .map((d) => d.count)
