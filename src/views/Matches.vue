...
export default defineComponent({
  ...
  setup() {
    ...
    const unfinished = computed(() => matchStore.status === MatchStatus.onGoing);
    const gameMode = computed(() => matchStore.gameMode);
    const map = computed(() => matchStore.map);
    const mmr = computed<Mmr>(() => matchStore.mmr);

    const maps = computed<MapInfo[]>(() => {
    ...
    const disabledGameModes = computed(() => {
