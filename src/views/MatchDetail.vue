...
export default defineComponent({
  ...
  setup(props) {
    ...
    const matchDuration = computed(() => formatSecondsToDuration(match.value.durationInSeconds));
    const playedDate = computed(() => formatTimestampStringToDate(match.value.startTime));
    const ffaPlayers = computed<PlayerScore[]>(() => [ffaWinner.value, ...ffaLosers.value]);
    const gateWay = computed(() => Gateways[matchStore.matchDetail.match.gateWay]);
    const season = computed(() => matchStore.matchDetail.match.season ?? 1);
    const isCompleteGame = computed<PlayerScore[]>(() => matchStore.matchDetail.playerScores);
    const loading = computed(() => matchStore.loadingMatchDetail);

    const matchIsFFA = computed(() => {
      const ffaModes = [EGameMode.GM_FFA, EGameMode.GM_SC_FFA_4];
      return ffaModes.includes(matchStore.matchDetail.match.gameMode);
    });

    const isJubileeGame = computed(() => {
